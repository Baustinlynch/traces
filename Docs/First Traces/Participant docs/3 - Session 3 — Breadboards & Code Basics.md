# Session 3 — Breadboards & Code Basics

## Welcome back

Open Wokwi and keep working on your simulated circuits! You'll have the entire session to work on your circuits.

Here's a reminder of how a breadboard is wired up:

![Diagram of how a breadboard's rows and power rails connect](/images/breadboard-diagram.png)

For a full breakdown of how a breadboard's rows and power rails are wired, see [[Session 4 — From Simulation to Breadboard]].

## Common lines of code you should know

- `pinMode(pin#, INPUT or OUTPUT);` — initializes the pin as a way to give or receive information.
- `delay(#milliseconds);` — pauses the loop for the defined number of milliseconds.
- `if (__==__){_______}` — an if statement, allowing conditions to affect what is run.
- `analogWrite(pin#, value)` / `digitalWrite(pin#, value)` — defines the pin as high or low, or (for analog) a value between 0 and 255 for voltage application.

---

**Original slides:** [Traces: Guide Slides!](https://www.canva.com/d/qSRdPFCi5AAqwSx)